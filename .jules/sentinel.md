...
## 2024-03-20 - Fix Command Injection in PowerShell Scripts
**Vulnerability:** The application executed PowerShell scripts by directly concatenating user-controlled file paths using string interpolation (`'${tempFile.replace(/\/g, '\\')}'`) inside `execAsync` calls for `upload` and `download` commands. This allowed arbitrary shell commands to be executed if `tempFile` contained malicious payloads.
**Learning:** Shell command execution methods like `exec` and `execAsync` evaluate full command strings within a shell environment (like PowerShell). Passing dynamic input directly into the command string creates a command injection vulnerability because the shell parses the entire string, interpreting special characters or commands.
**Prevention:** Never use string interpolation for dynamic arguments to shell commands or scripts. Pass data securely using environment variables (e.g., via the `env` options object) and reference them natively in the shell context (e.g., `$env:VAR_NAME` in PowerShell).
