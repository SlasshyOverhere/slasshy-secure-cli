...
## 2024-03-22 - Adding Global Keyboard Shortcuts for Discoverability and Accessibility
**Learning:** When adding global keyboard shortcuts to inputs, it's not enough to simply add a `keydown` listener. Sighted users need the shortcut discoverable (e.g., in a placeholder `(/)`), and screen reader users need it announced (e.g., in an `aria-label` like `Press / to focus`). Furthermore, the shortcut must only trigger when it won't interfere with ongoing typing, necessitating an explicit check of `document.activeElement.tagName` against `INPUT`, `TEXTAREA`, and `SELECT`.
**Action:** When adding global shortcuts, consistently update the corresponding element's placeholder and `aria-label`. Always check the `document.activeElement` to ignore the keystroke if the user is currently typing in an input field, and ensure to call `ev.preventDefault()` to prevent the shortcut key from being inadvertently typed into the newly focused element.
