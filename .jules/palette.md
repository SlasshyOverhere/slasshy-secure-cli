...
## 2025-04-01 - Global Keyboard Shortcuts and Discoverability
**Learning:** When adding single-key global keyboard shortcuts (like `/` for search) in the Web UI, the shortcut must be visibly discoverable (via placeholder text) and accessible (via `aria-label`). Additionally, the event listener must carefully check `document.activeElement.tagName` to avoid capturing keystrokes meant for active inputs (`INPUT`, `TEXTAREA`, `SELECT`), and `ev.preventDefault()` must be called to prevent the shortcut character from being typed into the newly focused element.
**Action:** Always append the keyboard shortcut hint to the input's placeholder (e.g., `Search… (/)`) and update the `aria-label` (e.g., adding `Press / to focus`) to ensure it is discoverable for sighted and screen reader users alike. When implementing the listener, check the active element's tag name and use `ev.preventDefault()` before programmatic focus.
