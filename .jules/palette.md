...
## 2024-03-25 - Global single-key shortcuts accessibility
**Learning:** Adding single-key global keyboard shortcuts (like `/` for search) can improve navigation speed, but if they intercept typing inside text fields (like `INPUT`, `TEXTAREA`, or `SELECT`), they create frustrating accessibility and usability regressions.
**Action:** Always check the `document.activeElement.tagName` and ignore single-key global shortcut keystrokes if the user is currently typing in an input field. Also ensure to call `ev.preventDefault()` so the shortcut character is not inadvertently typed into the focused element.
