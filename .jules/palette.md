...
## 2024-03-20 - Empty State Filter Recovery & Search UX
**Learning:** When search queries or type filters result in an empty list, displaying a static "No entries match your search" message leaves users without a direct path to recover. Additionally, search inputs lack standard keyboard affordances like pressing `Escape` to quickly clear the text and blur the input, adding friction to the navigation experience.
**Action:** Enhance empty state messages by injecting actionable elements (like a "Clear Filters" button) into the DOM that reset the active filters, and attach `keydown` listeners to search inputs to clear their value and remove focus when `Escape` is pressed.
