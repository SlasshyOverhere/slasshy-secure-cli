...
## 2024-03-20 - Empty State Recovery & Search Accessibility
**Learning:** When displaying empty states due to active filters or search queries, users may feel stuck if they have to manually clear multiple inputs to reset the view. Additionally, small search inputs lack quick keyboard resets (like `Escape`), and dynamic empty state changes in SPAs are often missed by screen readers.
**Action:** Always include actionable UI elements (like a "Clear Filters" button) in filtered empty states to provide a direct recovery path. Add `Escape` keydown listeners to search inputs to quickly clear and blur them, and apply `aria-live="polite"` to containers that render dynamic empty state text (like entry lists and detail hints) to ensure screen readers announce the changes.
