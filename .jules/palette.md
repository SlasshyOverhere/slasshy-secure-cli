...
## 2024-03-30 - Keyboard Shortcuts Accessibility
**Learning:** When adding keyboard shortcuts, sighted users and screen reader users must both be informed. Adding the shortcut to the input placeholder helps sighted users, while appending instructions to the `aria-label` ensures screen reader users also discover the functionality. Furthermore, the event listener must check the `tagName` of the `document.activeElement` to prevent the shortcut key from overriding legitimate typing in text areas or inputs.
**Action:** Always pair a visible indicator (like `placeholder="Search… (/)"`) with an accessible indicator (`aria-label="Search entries (Press / to focus)"`) when adding keyboard shortcuts. Verify the `activeElement` before calling `ev.preventDefault()` to ensure the shortcut does not hijack normal input.
