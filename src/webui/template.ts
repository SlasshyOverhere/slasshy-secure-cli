...
<body>
  ...
  <script type="module" nonce="${nonce}">
    ...
    document.addEventListener('keydown',ev=>{if(ev.key==='Escape'&&!el.videoModal.classList.contains('hidden')){closeVideoPreview()}else if(ev.key==='/'&&document.activeElement&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)){ev.preventDefault();el.search.focus()}});
