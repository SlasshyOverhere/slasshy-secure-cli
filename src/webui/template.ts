...
<body>
  ...
  <script type="module" nonce="${nonce}">
    ...
    function switchDetail(type,canWatchVideo){const note=type==='note',file=type==='file';el.detailPwd.classList.toggle('hidden',note||file);el.detailNote.classList.toggle('hidden',!note);el.detailFile.classList.toggle('hidden',!file);el.saveDetail.disabled=file;el.saveDetail.title=file?'File metadata is read-only.':'';el.copyPassword.classList.toggle('hidden',note||file);el.downloadFile.classList.toggle('hidden',!file);el.watchVideo.classList.toggle('hidden',!canWatchVideo);if(!canWatchVideo)closeVideoPreview()}
    ...
    function renderEntries(){
      ...
      s.entries.forEach(en=>{
        ...
        b.setAttribute('aria-label', (en.favorite?'Favorite, ':'') + en.title + ', ' + ty + (en.category?', Category '+en.category:'') + ', Modified ' + d.textContent);
