...
<body>
  <div class="app">
    ...
    <section class="main-grid">
      ...
      <aside class="card sidebar">
        ...
        <div class="filters">
          <input id="search" type="search" placeholder="Search entries… (/)" aria-label="Search entries (Press / to focus)">
  ...
  <script type="module" nonce="${nonce}">
    ...
    document.addEventListener('keydown',ev=>{
      if(ev.key==='Escape'&&!el.videoModal.classList.contains('hidden')){closeVideoPreview();return;}
      if(ev.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName||'')){ev.preventDefault();el.search.focus();}
    });
